...
#!/bin/sh
# PODCOP_SUB_V666_SAFE_PODKOP_RESTART_V392
...
case "$CMD" in
  start|restart|reload)
    ...
    if ! has_outbound; then
      logger -t podcop-sub-v666 "v392: skip podkop $CMD, no outbound configured yet"
      echo "podcop-sub-v666 v392: skip podkop $CMD, no outbound configured yet"
...
# SUBSYNC_PUBLIC_BUILD_V392
# SUBSYNC_PUBLIC_BUILD_V392
# SUBSYNC_INSTALL_VERSION_FILES_V392_BEGIN
set -u
...
echo "========================================="
echo "  Podcop Sub v666 — public install v392"
echo "========================================="
echo "Backup: disabled for public/friend install"
echo "Downloader: wget strict direct installer v392"
...
echo "=== fix LuCI static JS permissions v392 ==="
...
grep -v '/usr/bin/podcop-sub-v666-guard' /etc/crontabs/root > /tmp/root.cron.v392 2>/dev/null || true
echo '*/5 * * * * /usr/bin/podcop-sub-v666-guard >/tmp/podcop-sub-v666-guard.log 2>&1' >> /tmp/root.cron.v392
cat /tmp/root.cron.v392 > /etc/crontabs/root
rm -f /tmp/root.cron.v392
/etc/init.d/cron restart 2>/dev/null || true

echo "=== version ==="
echo "v392" > /etc/sub-sync/module-version
...
echo "DONE_MODULE_OK: Podcop Sub v666 v392 module installed."
echo "DONE_THEME_STATUS: mediaurlbase=$(uci get luci.main.mediaurlbase 2>/dev/null || true)"
echo "DONE: install.sh v392 finished rc=0"
# SUBSYNC_INSTALL_VERSION_FILES_V392_END
