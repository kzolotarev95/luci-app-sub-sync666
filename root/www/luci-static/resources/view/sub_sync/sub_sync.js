...
(function() {
  var STYLE_ID = 'ss-donaters-public-cards-v136-inline-style';

  function injectSubSyncV136Style() {
    ...
    style.id = STYLE_ID;
    style.textContent = [
      '@keyframes ssDonatersTitlePulseV136{0%,100%{filter:brightness(1)}50%{filter:brightness(1.28)}}',
      '@keyframes ssPodcopSubV666TitleShineV136{0%{background-position:0% 50%}50%{background-position:100% 50%}100%{background-position:0% 50%}}',
      '.ss-subsync-shine-v115,.ss-podcop-sub-v666-shine-v122{background:linear-gradient(90deg,#ff8a00,#ffd000,#00d4ff,#7c4dff,#ff4d9d);background-size:300% 300%;-webkit-background-clip:text;background-clip:text;color:transparent!important;animation:ssPodcopSubV666TitleShineV136 5s ease infinite,ssDonatersTitlePulseV136 2.8s ease-in-out infinite;font-weight:800;text-decoration:none!important;}',
      '.ss-donaters-public-cards-v136{border:1px solid rgba(255,255,255,.18)!important;border-radius:16px!important;padding:12px!important;margin:10px 0!important;background:linear-gradient(135deg,rgba(255,138,0,.12),rgba(124,77,255,.10))!important;box-shadow:0 8px 24px rgba(0,0,0,.12)!important;}'
    ].join('\n');

    (document.head || document.documentElement).appendChild(style);
  }

  function applySubSyncV136Inline() {
    if (typeof document === 'undefined')
      return;

    injectSubSyncV136Style();

    var links = document.querySelectorAll('a[href*="t.me"],a[href*="telegram"]');
    for (var i = 0; i < links.length; i++) {
      links[i].classList.add('ss-subsync-shine-v115');
      links[i].classList.add('ss-podcop-sub-v666-shine-v122');
      links[i].setAttribute('data-subsync-v136', 'SUBSYNC_DONATERS_PUBLIC_CARDS_V136_INLINE_STYLE');
    }

    var nodes = document.querySelectorAll('.cbi-section,.cbi-value,.panel,.card,div');
    for (var j = 0; j < nodes.length && j < 350; j++) {
      ...
      if (
        ...
        (
          ...
          t.indexOf('donater') >= 0 ||
          t.indexOf('поддерж') >= 0 ||
          t.indexOf('telegram') >= 0
        )
      ) {
        nodes[j].classList.add('ss-donaters-public-cards-v136');
        nodes[j].style.borderRadius = '16px';
        nodes[j].style.boxShadow = '0 8px 24px rgba(0,0,0,.12)';
      }
    }
  }
  ...
  if (typeof document !== 'undefined') {
    if (document.readyState === 'loading')
      document.addEventListener('DOMContentLoaded', applySubSyncV136Inline, { once: true });
    else
      setTimeout(applySubSyncV136Inline, 0);

    setTimeout(applySubSyncV136Inline, 400);
    setTimeout(applySubSyncV136Inline, 1200);
  }
})();
'use strict';
'require view';
