...
![Support](https://img.shields.io/badge/support-ended-lightgrey)
...
Я, текущий разработчик модуля, вынужден объявить, что **luci-app-sub-sync666** больше не будет обновляться и поддерживаться.

Изначально модуль был создан другим разработчиком. Я взял его в доработку, исправил ряд проблем и добавил функционал. Однако дальнейшая разработка стала нецелесообразной по нескольким причинам:

- У модуля очень запутанная и устаревшая структура кода;
- Большое количество багов, которые требуют серьёзной переработки практически всего модуля;
- Постоянное противодействие и «палки в колёса» со стороны команды Podcop.

В связи с этим я принял решение прекратить поддержку проекта.

## Репозиторий

Репозиторий остаётся в открытом доступе:

 **https://github.com/kzolotarev95/luci-app-sub-sync666**

Все желающие могут использовать последнюю версию или создать форк для дальнейшей разработки.

Спасибо всем, кто пользовался модулем. Извиняюсь перед теми, кто рассчитывал на дальнейшие обновления.
